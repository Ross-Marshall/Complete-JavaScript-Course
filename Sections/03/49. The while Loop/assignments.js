...
const myCountry = { country: country,
                    capital: capital,
                    language: language,
                    population: population,
                    neightbors: neighbors,
                    describe: function () {
                        console.log(`--- ${myCountry.country} has ${myCountry.population} people ${myCountry.neightbors.length} neighboring countries and a captial called ${myCountry.capital}, ---`);
                        return;
                    }
                };
...
for (let i = 0 ; i < listOfNeighbors.length ; i++) {
    let neighborList = listOfNeighbors[i];
    for (let j = 0 ; j < neighborList.length ; j++) {
        console.log(`Neighbor: ${neighborList[j]}`);
    }
}

let i = 0;
let j = 0
while  (i < listOfNeighbors.length) {
    let neighborList = listOfNeighbors[i];
    j = 0;
    while (j < neighborList.length) {
        console.log(`Neighbor: ${neighborList[j]}`);
        j++
    }
    i++;
